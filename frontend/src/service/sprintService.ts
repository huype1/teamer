...
const baseUrl = `${config.getApiBaseUrl()}/sprints`;
...
export const getBacklogIssues = async (projectId: string) => {
  const res = await axios.get(`${baseUrl}/project/${projectId}/backlog`, addReqToken(localStorage.getItem("token")));
  return res.data;
};

export const getActiveSprintIssues = async (projectId: string) => {
  const res = await axios.get(`${baseUrl}/project/${projectId}/active`, addReqToken(localStorage.getItem("token")));
  return res.data;
};

export const getUpcomingSprintIssues = async (projectId: string) => {
  const res = await axios.get(`${baseUrl}/project/${projectId}/upcoming`, addReqToken(localStorage.getItem("token")));
  return res.data;
};
